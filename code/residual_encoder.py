...
class ResidualEncoder(object):
    ...
    def build(self, input_data, vgg, is_training):
        ...
        if debug:
            ...
            assert b_conv1.get_shape().as_list()[1:] == [224, 224, 32]

        # Backward upscale layer 1 and add input layer
        b_conv1_add_pyramid = PyramidPoolingModule()(b_conv1)
        # bn_0 = self.batch_normal(input_data, "bn_0", is_training)
        # b_conv0_input = tf.add(bn_0, b_conv1_add_pyramid, name="b_conv0_input")
        # b_conv0 = self.conv_layer(b_conv0_input, "b_conv0", is_training)
        b_conv0 = self.conv_layer(b_conv1_add_pyramid, "b_conv0", is_training)

        if debug:
            # assert bn_0.get_shape().as_list()[1:] == [224, 224, 36]
            # assert b_conv0_input.get_shape().as_list()[1:] == [224, 224, 36]
